...
class ruderlage_Sensor:
        ...
        def _initialize_sensor(self):
                ...
                try:
                        ...
                        self.i2c_bus.write_byte(self.sensor_address, 0x00)
                        self.i2c_bus.write_byte(self.sensor_address, 0x00)
                        logging.info("Sensor Ruderlage erfolgreich initialisiert.")
                except Exception as e:
                        logging.error("Sensor Ruderlage nicht initialisiert {str(e)}")
                        raise
        ...
        def read_sensor(self):
                ...
                if (value >= 32768):
                        value = 65536 -value
                VRef = 2.048 # 2.048 Volts               #setSample(rate)
                k = .051
                N = 12 # resolution,number of bits
                Winkel = (((((2 * VRef* value)/ (2**N))*1000-931)*k)-0.6)/10
                 #return Winkel
                if self.debug:
                       logging.debug("SENSOR:\truderlage\tWinkel %f", Winkel)
                return Winkel*-1
