...
class FishPiConfig(object):
    ...
    def __init__(self):
        ...
        try:
            ...
            #self.ruder_sensor = ruderlage_Sensor(i2c_bus=self.shared_i2c_bus, debug=True)
            logging.info("Drive_Controller erfolgreich initialisiert.")
        except Exception as e:
            logging.error(f"Fehler bei der Initialisierung des Drive_Controllers: {e}")
            self.drive_controller = None  # Als Fallback auf None setzen
            #self.ruder_sensor = None  # Als Fallback auf None setzen
    ...
    def lookup(self, addr, debug=False):
        ...
        #elif addr == 0x68:
            #try:
            #    from sensor.ruderausschlag import ruderlage_Sensor
            #    self.ruder_sensor = ruderlage_Sensor(i2c_bus=self.shared_i2c_bus, debug=self.debug)
            #    logging.info("CFG:\tRuder sensor initialized.")
            #except Exception as ex:
            #    logging.warning("CFG:\tError setting up Ruder sensor - %s" % ex)

        elif addr == 0x68:
            try:
                from sensor.ruderlage import ruderlage_Sensor
                self.ruder_sensor = ruderlage_Sensor(i2c_bus=self.shared_i2c_bus, debug=self.debug)
                logging.info("CFG:\tRuderlagenanzeiger erfolgreich initialisiert.")
            except Exception as ex:
                logging.warning(f"CFG:\tFehler beim Initialisieren des Ruderlagenanzeigers - {ex}")
                self.ruder_sensor = None  # Fallback setzen
